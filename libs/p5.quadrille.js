  ...
  */
  ...
  _conv(mask, row, col, cache_half_size = (mask.width - 1) / 2) {
    if (row >= cache_half_size && col >= cache_half_size &&
      ...
      for (let imask = 0; imask < mask.height; imask++) {
        for (let jmask = 0; jmask < mask.width; jmask++) {
          ...
          if ((neighbour instanceof p5.Color || Array.isArray(neighbour)) &&
            ...
            let weight = typeof mask_value === 'number' ? mask_value : 0.299 * p5.red(mask_value) + 0.587 * p5.green(mask_value) + 0.114 * p5.blue(mask_value);
            r += p5.red(neighbour) * weight;
            g += p5.green(neighbour) * weight;
            b += p5.blue(neighbour) * weight;
          }
        }
      }
      ...
      this._memory2D[row][col] = p5.color(r, g, b);
    }
  }
    ...
    for (let i = 0; i < clone.height; i++) {
      for (let j = 0; j < clone.width; j++) {
        if (clone._memory2D[i][j]) {
          let _i, _j;
          do {
            _i = p5.int(p5.random(this.height));
            _j = p5.int(p5.random(this.width));
          }
  ...
  clear() {
    if (arguments.length === 0) {
      this._memory2D = this._memory2D.map(x => x.map());
    }
    ...
    switch (mode) {
      case 'DISTANCE':
        memory1D.sort((cellA, cellB) => {
          ...
          let wa = Math.sqrt(Math.pow((sa.r / sa.total) - p5.red(target), 2) + Math.pow((sa.g / sa.total) - p5.green(target), 2) +
            Math.pow((sa.b / sa.total) - p5.blue(target), 2) + Math.pow((sa.a / sa.total) - p5.alpha(target), 2));
          let wb = Math.sqrt(Math.pow((sb.r / sb.total) - p5.red(target), 2) + Math.pow((sb.g / sb.total) - p5.green(target), 2) +
            Math.pow((sb.b / sb.total) - p5.blue(target), 2) + Math.pow((sb.a / sb.total) - p5.alpha(target), 2));
          return wa - wb;
        });
        break;
      case 'AVG':
  ...
  static sample({ cell, charColor = 'black', background = this.BACKGROUND, cellLength = this.CELL_LENGTH, numberColor = this.numberColor, min = 0, max = 0 } = {}) {
    let r, g, b, a;
    let pg = p5.createGraphics(cellLength, cellLength);
  ...
  static NUMBER({
    ...
    if (min < max) {
      ...
      graphics.fill(graphics.color(p5.red(numberColor), p5.green(numberColor), p5.blue(numberColor), graphics.map(cell, min, max, 0, 255)));
      graphics.rect(0, 0, cellLength, cellLength);
      graphics.pop();
    }
    if (outlineWeight !== 0) {
...
export default function quadrille () {
